...
def basic_training_parameters(parser):
    ...
    parser.add_argument('--separate_bn_lr',    action='store_true',                help='Flag. If set, BN layer learning rates will be used.')
    parser.add_argument('--main_bn_lr',        default=0.00001,  type=float,        help='Learning Rate for main layer parameters.')
    parser.add_argument('--aux_bn_lr',         default=0.0000001,  type=float,        help='Learning Rate for auxiliary BN layer parameters.')
    ...
    parser.add_argument('--n_epochs',          default=125,      type=int,          help='Number of training epochs.')
    parser.add_argument('--kernels',           default=12,        type=int,          help='Number of workers for pytorch dataloader. (see formula in the main.py script)')
    ...
    parser.add_argument('--loss',            default='s2sd',      type=str,   help='Training criteria: For supported methods, please check criteria/__init__.py')
    ...
    parser.add_argument('--arch',             default='resnet50_normalize',  type=str,   help='Underlying network architecture. Frozen denotes that \
    ...
    parser.add_argument('--eval_params',            default='cub200_multisimilarity',      type=str,   help='Shared MDProp parameters to verify performance. (cub200_multisimilarity, cub200_s2sd, cars196_multisimilarity, sop_multisimilarity)')

    ##### Setup Parameters
    parser.add_argument('--gpu',          default='0', nargs='+',                  type=str, help='Gpu to use.-1 to use all gpus')
    ...
    parser.add_argument('--source_path',  default='/path/to/data',   type=str, help='Path to training data.')
...
def attack_parameters(parser):
    parser.add_argument('--attacker1_targets', default=1, type=int, help='Greater than 1 for our MasterProp.')
    parser.add_argument('--attacker2_targets', default=5, type=int, help='Greater than 1 for our MasterProp.')
    parser.add_argument('--epsilon', default=0.01, type=float, help='l-infinity bound for adversarial noise.')
    parser.add_argument('--attack_itr', default=1, type=int, help='Number of attack iterations.')
    return parser
